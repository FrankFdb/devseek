...
`CLOSE-01` 与 `CLOSE-02-SAME-WINDOW-SURFACE-RECEIPT` 已完成当前产品运行层、历史结算层与 cleanup/applicability 收口；`G0-01-ACTIVE-BASELINE-SELECTOR`、`G0-02-LEGACY-DOC-INVENTORY`、`G0-03-FRONTMATTER-BANNER-GENERATION` 与 `G0-04-PROFILE-DENOMINATOR-REGISTRY` 已新增机器 source/schema/checker/generated view 与对应 Phase gate。GPT-5.5 后继首选卡是 `G0-05-PROFILE-EXECUTOR-CONFORMANCE`：

- 为 G0-04 已冻结的五类 slot semantics 建立真实 executor contract，经唯一 runner root 调度；production entry 仍可 fail closed，不得把 catalog metadata 当执行。
- 先写最小失败测试：空输入、替换 case、错 slot/attempt、缺 receipt、未授权外部动作与 executor registry 漂移都必须 fail closed。
- 只实现 runner adapter/executor registry/inventory/tests；不改产品 Kernel，不运行 Provider/live/holdout，不提升 claims，不进入 protected qualification。
- focused + generated drift + Phase 全绿，独立 P0/P1=0 后一个 commit；G0-05 PASS 后才可领取需要真实 identity reader 的 `G0-06-CURRENT-CANDIDATE-IDENTITY-PROBE` 或后续 C0 wiring 卡。
- `done_iff`：executor coverage=`100%`；每个已声明语义有独立 oracle；缺失/错轮/替换/无 receipt/外部动作 attack 均 veto；qualification effect=`NONE`。
