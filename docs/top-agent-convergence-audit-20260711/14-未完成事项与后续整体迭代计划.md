...
`CLOSE-01` 与 `CLOSE-02-SAME-WINDOW-SURFACE-RECEIPT` 已完成当前产品运行层、历史结算层与 cleanup/applicability 收口；`G0-01-ACTIVE-BASELINE-SELECTOR`、`G0-02-LEGACY-DOC-INVENTORY` 与 `G0-03-FRONTMATTER-BANNER-GENERATION` 已新增机器 source/schema/checker/generated view 与受管文档状态。GPT-5.5 后继卡是 `G0-04-PROFILE-DENOMINATOR-REGISTRY`：

- 为七个 C0 节点定义精确 profile/corpus/catalog/slot/applicability 分母；不得用 wildcard/default claim 或 catalog metadata 代替已执行结果。
- 先写最小失败测试：缺 denominator、重复 slot、attempt identity 漂移、profile applicability 被降分母、schema/runtime 双向攻击都必须 fail closed。
- 只实现 qualification profile/catalog schema、generator、攻击 fixture 与 drift checker；不改产品 Kernel、不运行 Provider/live/holdout、不提升 claims。
- focused + generated drift + Phase 全绿，独立 P0/P1=0 后一个 commit；G0-04 PASS 后才可进入 `G0-05-PROFILE-EXECUTOR-CONFORMANCE` 或 `G0-06-CURRENT-CANDIDATE-IDENTITY-PROBE`。
- `done_iff`：七个 C0 节点的 denominator、slot、attempt identity 与 applicability 守恒；无 wildcard/default claim；qualification effect=`NONE`。
