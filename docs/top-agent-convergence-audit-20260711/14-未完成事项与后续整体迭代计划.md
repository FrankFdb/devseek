...
- 当前结论：G0-A/B/C/D、一个本地非资格 runner 纵切、机器 decision contract 和 G0-01 Active Baseline selector 已实现；**Gate 0=`NOT_PASSED`、claims=`[]`、R1=`NOT_STARTED`**
...
| restricted entrypoint scan | 250 files；known forbidden authority imports 0 |
...
`CLOSE-01` 与 `CLOSE-02-SAME-WINDOW-SURFACE-RECEIPT` 已完成当前产品运行层、历史结算层与 cleanup/applicability 收口；`G0-01-ACTIVE-BASELINE-SELECTOR` 与 `G0-02-LEGACY-DOC-INVENTORY` 已新增机器 source/schema/checker/generated view。GPT-5.5 后继卡是 `G0-03-FRONTMATTER-BANNER-GENERATION`：

- 以 `docs/process/devseek-active-baseline-selector.json` 和 `docs/process/devseek-legacy-doc-inventory.json` 为唯一机器输入生成 front matter、superseded banner 与 README/status 最小视图；不得手工批量粘贴 banner。
- 先写最小失败测试：generated view stale、active baseline 被加 superseded banner、legacy inventory 条目缺 banner、README/status 与 machine source 不一致、生成器覆盖正文都必须 fail closed。
- 只实现 docs generator、approved docs header/banner/status 更新和 drift checker；不改产品 Kernel、不删除历史证据、不把 Markdown 反向作为机器 authority。
- focused + second generation diff=0 + architecture + Phase 全绿，独立 P0/P1=0 后一个 commit；文档-only/治理-only不需要虚构 live 结果。
- `done_iff`：Active 与 historical 链接无冲突，所有 governed legacy 文档的 banner/status 均可由 manifest 重生成且二次运行 diff=0；下一卡按第 3.2 节依赖为 `G0-04-PROFILE-DENOMINATOR-REGISTRY` 或并行申请外部 authority，不得跳过 G0-03。
