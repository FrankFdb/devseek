...
release_identity: pending this working-tree source commit and local VSIX package/install loop.
controlled_vsix_self_loop_status: PENDING_RELEASE_LOOP; existing VS Code windows were not closed.
...
release_identity: pending this working-tree source commit and local VSIX package/install loop.
controlled_vsix_self_loop_status: PENDING_RELEASE_LOOP; fixed old simulation cases are not reused as generic proof. R2-02B adds a new weak-oracle simulation case tied to this defect class.
...
release_identity: pending this working-tree source commit and local VSIX package/install loop.
controlled_vsix_self_loop_status: PENDING_RELEASE_LOOP; R2-02C adds an external-boundary simulation case rather than relying on the fixed controlled/self-loop suite alone.
