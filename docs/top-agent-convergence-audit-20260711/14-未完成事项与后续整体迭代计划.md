...
release_identity: PENDING_SOURCE_COMMIT_AND_VSIX_INSTALL. Extension/Bridge behavior changed, so the next step is source/docs commit, debug VSIX packaging, packaged Bridge verify, local install, active runtime identity check, and VSIX self-loop without closing existing VS Code windows.
