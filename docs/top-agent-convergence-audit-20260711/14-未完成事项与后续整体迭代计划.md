...
release_identity: TO_PRODUCE_AFTER_COMMIT
controlled_vsix_self_loop_status: TO_RUN_AFTER_RELEASE_INSTALL_WITH_WINDOW_RETAINED
next_atomic_id: package/install and rerun exact VSIX `realistic-product`; if PASS, proceed by user instruction; if FAIL, record the next concrete defect class first.
```
