...
- 当前结论：G0-A/B/C/D、一个本地非资格 runner 纵切、机器 decision contract 和 G0-01～10 机器治理迭代已实现；**Gate 0=`NOT_PASSED`、claims=`[]`、R1=`NOT_STARTED`**
...
| restricted entrypoint scan | 259 files；known forbidden authority imports 0 |
...
Repository blockers 已由 G0-10 归零，但这只代表七个 C0 本地实现/接线门均有机器证据；不得把它解释为 Gate 0 通过。Gate 0 仍被 6 个外部 authority blocker 与 0/7 exact protected claims 阻塞，必须由独立授权、受保护身份/设施、retention/time/attestation 和精确 tuple claims 解决。
...
`CLOSE-01` 与 `CLOSE-02-SAME-WINDOW-SURFACE-RECEIPT` 已完成当前产品运行层、历史结算层与 cleanup/applicability 收口；`G0-01-ACTIVE-BASELINE-SELECTOR`、`G0-02-LEGACY-DOC-INVENTORY`、`G0-03-FRONTMATTER-BANNER-GENERATION`、`G0-04-PROFILE-DENOMINATOR-REGISTRY`、`G0-05-PROFILE-EXECUTOR-CONFORMANCE`、`G0-06-CURRENT-CANDIDATE-IDENTITY-PROBE`、`G0-07-C0-LEDGER-WIRING`、`G0-08-C0-PREREGISTRATION-WIRING`、`G0-09-C0-RUN-EVIDENCE-WIRING` 与 `G0-10-C0-MANIFEST-AGGREGATOR-WIRING` 已新增机器 source/schema/checker/generated view 与对应 Phase gate。GPT-5.5 后继首选卡是 `G0-11-EXTERNAL-AUTHORITY-ADAPTER-CONTRACT`：

- 只实现外部 attestation/trusted registry/retention/time authority 的 fail-closed import contract、schema、reader/checker 和 negative oracle；不得导入真实 secret、不得调用 Provider/live/holdout、不得把本地 self-hash 或普通 object/boolean 当作受保护 trust。
- 先写最小失败测试：普通对象伪造 attestation、boolean pass、撤销后复用、角色/密钥用途复用、clock rollback、manifest/provenance mismatch、缺 external retention anchor、缺 trust root、fallback 到 local fixture/self-hash，都必须 fail closed。
- allowed：adapter interfaces、receipt/import schemas、verification policy/checker、generated view、capability ledger component/verification command、Phase gate 和相关 deterministic tests。no-touch：产品 Kernel、用户窗口、Provider/账号/网络/live/holdout、qualification claims、外部真实基础设施。
- focused + affected full + generated drift + Phase 全绿，独立 P0/P1=0 后一个 commit；G0-11 PASS 只证明外部 authority adapter contract 已 fail-closed 接入，不代表外部 authority 已存在。
- `done_iff`：无 trust root 时 Gate 0 仍 `NOT_PASSED`；普通 object/boolean 不能伪造外部 evidence；revocation、role reuse、time rollback、manifest/provenance mismatch 均被拒绝；qualification effect=`NONE`，claims 仍只能由受保护 exact tuple evidence 产生。
