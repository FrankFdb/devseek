...
- 当前结论：G0-A/B/C/D、一个本地非资格 runner 纵切、机器 decision contract 和 G0-01～11 机器治理迭代已实现；**Gate 0=`NOT_PASSED`、claims=`[]`、R1=`NOT_STARTED`**
...
| restricted entrypoint scan | 260 files；known forbidden authority imports 0 |
...
Repository blockers 自 G0-10 起归零，并经 G0-11 external adapter contract 接入后保持 0；这只代表七个 C0 本地实现/接线门均有机器证据，不得把它解释为 Gate 0 通过。Gate 0 仍被 6 个外部 authority blocker 与 0/7 exact protected claims 阻塞，必须由独立授权、受保护身份/设施、retention/time/attestation 和精确 tuple claims 解决。
...
`CLOSE-01` 与 `CLOSE-02-SAME-WINDOW-SURFACE-RECEIPT` 已完成当前产品运行层、历史结算层与 cleanup/applicability 收口；`G0-01-ACTIVE-BASELINE-SELECTOR`、`G0-02-LEGACY-DOC-INVENTORY`、`G0-03-FRONTMATTER-BANNER-GENERATION`、`G0-04-PROFILE-DENOMINATOR-REGISTRY`、`G0-05-PROFILE-EXECUTOR-CONFORMANCE`、`G0-06-CURRENT-CANDIDATE-IDENTITY-PROBE`、`G0-07-C0-LEDGER-WIRING`、`G0-08-C0-PREREGISTRATION-WIRING`、`G0-09-C0-RUN-EVIDENCE-WIRING`、`G0-10-C0-MANIFEST-AGGREGATOR-WIRING` 与 `G0-11-EXTERNAL-AUTHORITY-ADAPTER-CONTRACT` 已新增机器 source/schema/checker/generated view 与对应 Phase gate。GPT-5.5 后继首选卡是 `G0-12-C0-WIRING-RECONCILIATION`：

- 只做 C0 wiring 与 machine decision reconciliation：逐项复算 G0-07～11 的 governed consumer、source hash、schema/checker/package script/Phase gate、generated view drift 和 Gate0 decision source binding；不得新增产品功能、不得运行 Provider/live/holdout、不得生成 claims。
- 先写或加固最小失败 oracle：人工调 ledger state、缺任一 governed consumer、缺 generated/source/checker、外部 blocker 被本地删除、claims 被 fixture/Markdown 写入、fresh clone/regeneration drift，都必须 fail closed。
- allowed：machine ledger generator/checker/report、现有 process schema/generated view、Phase gate reconciliation、deterministic tests。no-touch：产品 Kernel、用户窗口、Provider/账号/网络/live/holdout、真实外部 authority、qualification claims、R1/R2/R3 实现。
- focused + affected full + generated drift + Phase 全绿，独立 P0/P1=0 后一个 commit；G0-12 PASS 只证明本地 C0 wiring/decision 全量收敛，不代表外部 authority、protected profile execution 或 Gate 0 已通过。
- `done_iff`：fresh clone/重生成一致；repository blockers 只能随可复算证据保持 0；external blockers 仍为真实 6；claims=`[]`、Gate 0=`NOT_PASSED`、R1=`NOT_STARTED`；任何手工 state、Markdown、fixture 或 local PASS 都不能降低 blocker。
