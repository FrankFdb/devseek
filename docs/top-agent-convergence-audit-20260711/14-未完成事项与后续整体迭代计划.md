...
`CLOSE-01` 与 `CLOSE-02-SAME-WINDOW-SURFACE-RECEIPT` 已完成当前产品运行层、历史结算层与 cleanup/applicability 收口；`G0-01-ACTIVE-BASELINE-SELECTOR`、`G0-02-LEGACY-DOC-INVENTORY`、`G0-03-FRONTMATTER-BANNER-GENERATION`、`G0-04-PROFILE-DENOMINATOR-REGISTRY`、`G0-05-PROFILE-EXECUTOR-CONFORMANCE`、`G0-06-CURRENT-CANDIDATE-IDENTITY-PROBE` 与 `G0-07-C0-LEDGER-WIRING` 已新增机器 source/schema/checker/generated view 与对应 Phase gate。GPT-5.5 后继首选卡是 `G0-08-C0-PREREGISTRATION-WIRING`：

- 复核真实 profile executor 是否必经 preregistration plan、session、slot、authorization 与 receipt，production declaration 入口必须全部覆盖。
- 先写最小失败测试：missing/expired/replay/clock rollback receipt 的 dispatch 必须为 `0`；普通产品任务不得被误当 qualification；TOFU 与 durable registration 后置都必须 fail closed。
- 只实现 existing runner composition、plan adapters、entrypoint inventory 与对应机器视图；不改产品 Kernel，不运行 Provider/live/holdout，不提升 claims，不进入 protected qualification。
- focused + generated drift + Phase 全绿，独立 P0/P1=0 后一个 commit；G0-08 PASS 后才可领取第 3.2 节满足前置的后续 C0 wiring 卡。
- `done_iff`：production declaration 入口 coverage=`100%`；missing/expired/replay/clock rollback dispatch=`0`；qualification effect=`NONE`。
