...
`CLOSE-01` 与 `CLOSE-02-SAME-WINDOW-SURFACE-RECEIPT` 已完成当前产品运行层、历史结算层与 cleanup/applicability 收口；`G0-01-ACTIVE-BASELINE-SELECTOR`、`G0-02-LEGACY-DOC-INVENTORY`、`G0-03-FRONTMATTER-BANNER-GENERATION`、`G0-04-PROFILE-DENOMINATOR-REGISTRY`、`G0-05-PROFILE-EXECUTOR-CONFORMANCE` 与 `G0-06-CURRENT-CANDIDATE-IDENTITY-PROBE` 已新增机器 source/schema/checker/generated view 与对应 Phase gate。GPT-5.5 后继首选卡是 `G0-07-C0-LEDGER-WIRING`：

- 复核所有 governed selector/status 消费者是否只消费唯一 capability ledger schema，不从 Markdown、手写 state 或第二份状态文件反向驱动。
- 先写最小失败测试：ledger schema owner 不唯一、consumer bypass、generated view stale、未满足证据却升 `wired`、Markdown/state 反向写 ledger 都必须 fail closed。
- 只实现 ledger authority、generated views、import/reachability guards；不改产品 Kernel，不运行 Provider/live/holdout，不提升 claims，不进入 protected qualification。
- focused + generated drift + Phase 全绿，独立 P0/P1=0 后一个 commit；G0-07 PASS 后才可领取 `G0-08-C0-PREREGISTRATION-WIRING` 或第 3.2 节满足前置的后续 C0 wiring 卡。
- `done_iff`：ledger schema owner=`1`、governed consumers coverage=`100%`、bypass=`0`；只有可复算证据满足才将对应 capability/state 升 `wired`；qualification effect=`NONE`。
