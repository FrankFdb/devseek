...
`CLOSE-01` 与 `CLOSE-02-SAME-WINDOW-SURFACE-RECEIPT` 已完成当前产品运行层、历史结算层与 cleanup/applicability 收口；`G0-01-ACTIVE-BASELINE-SELECTOR` 已新增机器 selector、checker 和 generated view。GPT-5.5 后继卡是 `G0-02-LEGACY-DOC-INVENTORY`：

- 先只读盘点受治理 legacy 需求/设计/接管文档范围，以 G0-01 selector 为 Active Baseline 输入；使用定向 `rg`，不扫 backups/node_modules/dist/media/制品。
- 先写最小失败测试：遗漏文档、路径不存在、重复裁决、未知状态、active baseline 被当成 legacy、supporting refs 无反向覆盖都必须 fail closed。
- 只实现 migration inventory schema/checker/generated view；不改正文、不批量粘 banner、不删除历史证据、不修改产品 Kernel，不触网。
- focused + generated drift + architecture + Phase 全绿，独立 P0/P1=0 后一个 commit；文档-only/治理-only不需要虚构 live 结果。
- `done_iff`：所有受治理历史文档 coverage=100%，keep/revise/supersede/archive/N/A 裁决完整，Active Baseline 不冲突；下一卡为 `G0-03-FRONTMATTER-BANNER-GENERATION`。
