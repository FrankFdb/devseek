...
release_identity: pending post-source-commit compile/package/packaged-Bridge/SHA/install/current-candidate identity because Extension behavior changed. Existing VS Code windows must remain open; any active runtime mismatch is an identity/reload issue, not a DeepSeek login precondition.
