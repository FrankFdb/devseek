...
- 盘点范围：本目录 `README`、01～18，`docs/architecture` 01～18 的承接关系，以及 `docs/process` 机器账本、runner inventory、Gate 0 decision 和默认 Phase
...
当前第一个可领取 leaf 是 `CLOSE-01-ACTIVE-RUNTIME-IDENTITY`。任何需要用户窗口变更的动作未获 fresh authorization 时只能 `BLOCKED`；`CLOSE-02` 的 `NOT_RUN` 也只能 `BLOCKED`。两个 leaf 都 `PASS`（或用户明确取消相应门）后才能开始 G0-01。
...
按第 3.1 节已持久的快照，下一窗口唯一工作包是 `CLOSE-INTEGRATION-GATE0-LOCAL`，第一个可领取 leaf 是 `CLOSE-01-ACTIVE-RUNTIME-IDENTITY`。不得根据旧聊天假定 active runtime 或同窗口回执已成立；需要用户窗口动作却未获得新授权时以 `BLOCKED` 结算并停止。

只有 `CLOSE-01` 通过后才领取 `CLOSE-02-SAME-WINDOW-SURFACE-RECEIPT`；整个 CLOSE 工作包通过后，GPT-5.5 后继卡才是 `G0-01-ACTIVE-BASELINE-SELECTOR`：
