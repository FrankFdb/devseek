...
- 当前结论：G0-A/B/C/D、一个本地非资格 runner 纵切、机器 decision contract 和 G0-01～09 机器治理迭代已实现；**Gate 0=`NOT_PASSED`、claims=`[]`、R1=`NOT_STARTED`**
...
| restricted entrypoint scan | 258 files；known forbidden authority imports 0 |
| Gate 0 capabilities | 7 |
| implementation requirements | 5/7 satisfied；2 repository blockers |
...
两个 repository blockers 精确对应：`C0-QUALIFICATION-AGGREGATOR`、`C0-QUALIFICATION-EVIDENCE-MANIFEST`。不得手工把它们从 `implemented` 改成 `wired`；必须先有 reachability、bypass=0、运行证据和生成物门禁。
...
| 10 G0-A | `completed` / `implemented-but-unqualified` | 七节点已有 5 项 machine `wired`；aggregator/manifest 与外部 protected authority 仍未闭合 |
...
`CLOSE-01` 与 `CLOSE-02-SAME-WINDOW-SURFACE-RECEIPT` 已完成当前产品运行层、历史结算层与 cleanup/applicability 收口；`G0-01-ACTIVE-BASELINE-SELECTOR`、`G0-02-LEGACY-DOC-INVENTORY`、`G0-03-FRONTMATTER-BANNER-GENERATION`、`G0-04-PROFILE-DENOMINATOR-REGISTRY`、`G0-05-PROFILE-EXECUTOR-CONFORMANCE`、`G0-06-CURRENT-CANDIDATE-IDENTITY-PROBE`、`G0-07-C0-LEDGER-WIRING`、`G0-08-C0-PREREGISTRATION-WIRING` 与 `G0-09-C0-RUN-EVIDENCE-WIRING` 已新增机器 source/schema/checker/generated view 与对应 Phase gate。GPT-5.5 后继首选卡是 `G0-10-C0-MANIFEST-AGGREGATOR-WIRING`：

- 复核 qualification evidence manifest 是否由独立 aggregator 从冻结 denominator、attempt streams、veto、retention、identity/provenance 和 G0-09 run evidence correlation 逐项复算，而不是信任 writer summary 或任意 self-hash。
- 先写最小失败测试：writer forged summary、缺 denominator slot、失败 attempt 被删除或覆盖、veto 被忽略、retention/anchor/provenance 漂移、`.some()` 任一 profile 替代 exact claim binding、同仓 self-hash 被当 protected trust，都必须 fail closed。
- 只实现 aggregator wiring source/schema/checker/generated view、manifest policy adapter 与 reader/oracle tests；不改产品 Kernel，不运行 Provider/live/holdout，不提升 claims，不进入 external authority 或 protected qualification。
- focused + generated drift + Phase 全绿，独立 P0/P1=0 后一个 commit；G0-10 PASS 后才可领取第 3.2 节满足前置的后续 C0 wiring 卡。
- `done_iff`：denominator/failure/veto/retention/provenance 均由 aggregator 独立复算；writer summary 无效；qualification effect=`NONE`，Gate 0 仍只按受保护 exact claims 判定。
