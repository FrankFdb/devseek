...
2026-07-17 R1-D environment capability resolver planned card：

```text
task_id: R1-D-ENVIRONMENT-CAPABILITY-RESOLVER
terminal_state: PLANNED
qualification_scope_and_claim_effect: product-side nonqualification; qualification effect=NONE; claims=[]; Gate0=NOT_PASSED; qualification R1=NOT_STARTED
defect_class: DevSeek must not execute a model-suggested command merely because the model wrote it. The local runtime must first determine whether the host has the required capability, choose safe available equivalents, and ask the user before installation or privileged/environment-changing remediation.
intended_owner: add a single environment capability boundary before task execution, terminal validation and install suggestion UI; do not scatter `which python` style probes through planners or tool executors.
expected_behavior: derive required capabilities from intent, changed files and validation plan; probe local availability/version for runtimes such as `python3`, `node`, `g++`, package managers and test runners; normalize safe equivalents such as `python` -> `python3` when policy allows; if missing, offer explicit user choices: install with an exact command, switch to an available tool, generate/edit files without running, or stop with blocked validation.
codex_claude_alignment: LLM output is a proposal, not command authority. Deterministic local policy owns capability discovery, command normalization, permission prompts, missing-tool explanations and evidence settlement. Provider text cannot mark a missing capability as passed, and a failed capability probe should produce a clear actionable UI card rather than a generic red task failure.
oracle_required: tests must cover available equivalent selection, missing runtime blocked status, install-choice prompt without auto-install, no network/provider/live/holdout side effects, history rendering after reload, and controlled VSIX normal/exception/boundary cases.
```
