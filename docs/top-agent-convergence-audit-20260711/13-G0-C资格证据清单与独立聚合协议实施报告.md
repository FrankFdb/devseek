...
| live / natural-UI / holdout | **未运行；不得回填为 pass** |

Focused pass 不替代最终全量门禁，也不自动提升 implementation state 到 `wired`。

## 8. Gate 0 交接，不是 R1 开工许可

G0-A/B/C/D 的本地实现纵切已经存在，但 Gate 0 要求的是七个 C0 节点全部满足机器 profile 的 `wired/L2` 精确 tuple，而不是“四个工作包写完”。当前应进入 **Gate 0 资格缺口闭合与复核**，至少包括：

1. 把真实 qualification runner 的所有声明入口接到 G0-B guard/event/receipt 与 G0-C reader，证明不可绕过；
...
在这些条件实际满足前：Gate 0 保持未通过；没有 live；`qualification_claims=[]`、`qualification_level_view=null`；R1-KERNEL-DG01-03 和任何 candidate/stable/top-tier 宣称均被禁止。后续工作不再表述为“只实现 G0-C”，也不能绕过资格缺口直接开始 Kernel 迁移。
