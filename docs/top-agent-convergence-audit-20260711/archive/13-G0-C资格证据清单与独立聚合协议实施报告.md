...
devseek_governance:
  ...
  status: "historical"
  path: "docs/top-agent-convergence-audit-20260711/archive/13-G0-C资格证据清单与独立聚合协议实施报告.md"
...
> 归档于 2026-08-03。本文仅保留 G0-C 当时实施证据；当前状态以 `docs/process` 机器事实为准。
...
- 接管定位：历史本地资格协议实施证据；任务、身份分层和下一步只以 14～18 为准
...
| 最终 Phase 0～12 report | 按 [15 第 3 节](../15-新窗口与跨模型接管手册.md#3-动态基线与五类身份) 解析 report 的 source commit；它必须匹配 implementation commit，不要求匹配 docs-only handoff commit |
| 最终 VSIX 路径 / SHA-256 / installed identity | 按 15 分别复算 artifact source、stable install 与 active runtime；installed package 不能代替用户窗口实际加载身份 |
| Git identities | handoff doc commit、implementation commit、Phase source 和 artifact source 分栏报告；短 SHA 先唯一解析为 full commit |
| Surface / live / holdout | exact-VSIX controlled 已有本地证据；用户当前 DevSeek 窗口为 `NOT_RUN/AUTHORIZATION_NOT_CARRIED_FORWARD`；protected live/holdout **未运行** |
