...
- 当前结论：顶级编程智能体目标尚未达成；C0 本地实现闭环、Intent Semantic Contract v3，以及 VS Code 新任务、断点恢复和本地验证修复的 canonical Kernel 路由已完成，但 CLI/Headless 单一 Coding Kernel、五个跨 Surface 语义域、C1～C14 产品接线、完整用户仿真和受保护资格仍未完成。
...
| Extension 类型与候选包基线 | `completed` | TypeScript 基线零错误；162/162 extension 套件、架构守卫、48/48 自然输入与 8 步 Webview 人工输入仿真通过；精确 VSIX T3 Surface 仿真继续作为发版门禁 |
| Surface 入口盘点 | `completed` | 87/87 入口受 inventory 覆盖，未知入口与未声明 legacy owner 可达性均为 0 |
| Kernel owner 收敛基线 | `completed` | v3 将本地产品迭代与资格晋级解耦；3 条活跃路由中 VS Code fresh-task canonical route=1、canonical recovery route=1、legacy recovery route=0、CLI legacy route=1；legacy execution owner=1、跨 Surface Kernel route=0、5 个核心语义域收敛数=0、Headless 产品入口=0；38/38 源码与默认门禁断言通过 |
...
| 12 | `SURFACE-CONTRACT-01` | `pending` | `KERNEL-02` | 同一任务在 VS Code、CLI、Headless 产生等价的状态、工具、验证和完成结果；静态守卫防止新旁路 |
| 13 | `CAP-C1-C14-WIRING` | `pending` | `SURFACE-CONTRACT-01` | 按 capability DAG 将 C1～C14 的 69 项能力从 `proposed` 逐项提升到可证的 `implemented/wired`；每项均有 owner、产品入口、失败恢复和机器证据 |
| 14 | `USER-SIM-01` | `in_progress` | 每个产品切片 | 以用户方式覆盖安装包、真实入口、多轮任务、失败恢复和结果核验；当前自然输入与 Webview/Kernel 恢复仿真已通过，真实安装包多轮恢复和 real Provider 仍待相应前置 |
| 15 | `QUAL-EXT-01` | `blocked_external` | 独立授权、受保护身份/设施、holdout 和不可变保留 | 6 个外部 blocker 由授权主体关闭，7 个 C0 exact tuple claims 可复算，机器 decision 自主达到 `PASS`；只约束资格晋级，不阻塞本地产品迭代 |
| 16 | `TOP-AGENT-ACCEPTANCE-01` | `pending` | `CAP-C1-C14-WIRING`、`USER-SIM-01` | 按 01～09 的需求与黄金旅程，对照 Codex 和 Claude Code 在同类问题上的可观察行为；所有适用产品能力、长任务、故障恢复、结果验证和跨 Surface 验收通过 |
...
`DOC01-KERNEL-CLI-03`（`in_progress`，父任务 `KERNEL-02`）

- 当前条件：VS Code fresh task、checkpoint resume 与 local validation repair 均进入 canonical Kernel；VS Code legacy recovery route=0、legacy execution owner=0。CLI 仍由 `CliLegacyCodingLoop` 执行，Headless 产品入口仍缺失。
- 接下来需要：把 canonical Kernel request/output 与 VS Code Surface 细节解耦，建立 CLI adapter；让 CLI 通过同一 TaskContract、工具、mutation、verification 与 completion 边界执行，并删除 `CliLegacyCodingLoop` 的业务 owner 身份。
- 后续任务：接入 Headless 产品入口，再逐项收敛 TaskContract、ToolExecutor、mutation、verification 与 completion 五个跨 Surface 语义域。
- 当前验收：CLI legacy execution owner=0、CLI canonical route=1；CLI 与 VS Code 的 create/modify/repair/permission-denied/policy-refusal conformance 输出等价；全量、自然输入、CLI 用户仿真、精确 VSIX 用户路径和架构基线通过。
- 归档判断：01 的 VS Code 路由与恢复子责任已完成，但 CLI、Headless 和五个语义域仍未完成，因此 01 整份文档继续留在根目录；不得创建归档副本或尾页。
