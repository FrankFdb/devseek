...
| 20 | `USER-SIM-01` | `in_progress` | 每个产品切片 | 以用户方式覆盖安装包、真实入口、多轮任务、失败恢复和结果核验；当前自然输入、Webview/Kernel 恢复仿真及精确 VSIX 单轮写入/读回/QualityGate 已通过，真实安装包多轮恢复和 real Provider 仍待相应前置 |
