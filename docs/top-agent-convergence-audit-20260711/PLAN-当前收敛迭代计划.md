...
| C11：恢复幂等 | `in_progress` | shared `ResumeIdempotencyPort` 与 Headless 产品路径已实现 completed skip、indeterminate block、failed-no-effect retry 和防篡改回执；checkpoint 已显式封存 `effectClass` 与 exact `operationSha256`，external-effect receipt 只能结算或跳过同一 operation，替换 input 在 host dispatch 前 fail closed。因 workspace receipt 直接结算、VS Code/CLI 适用恢复路径和跨重启对账未全部闭环，仍保持 `implemented` |
