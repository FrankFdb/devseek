...
devseek_governance:
  ...
  path: "docs/architecture/01-顶级编程智能体总体架构设计.md"
  source_group: "architecture"
  decision: "active"
  relationship: "primary"
  document_id: "ARCH-01-TOP-AGENT-ARCHITECTURE-BASELINE"
...
16. 模型工具方言必须先经过 `ModelToolProtocolAdapter` 统一归一，再进入 `ToolCallNormalizer`、执行器、完整性检查和 UI 展示；不得在多个阶段重复解释原始模型文本。
...
flowchart TB
  ...
  subgraph Model["Model and Provider Plane"]
    ...
    ProtocolAdapter[ModelToolProtocolAdapter]
  ...
  ProviderRouter --> ProtocolAdapter
  Integrity --> ProtocolAdapter
...
1. **同类问题系统性修复。** 任意问题修复都必须从现象追溯到缺陷类型，审计同类入口、状态流、工具协议、权限、验证、恢复和 UI 边界；不得只修当前截图路径。若同类缺陷可能存在于多个 Surface、Provider、Agent 循环或本地执行路径，必须整体收敛。
2. **重复逻辑单一化。** 完全一样或语义等价的业务规则、协议判断、权限判断、消息净化、验证结论和恢复策略，原则上在项目中只能有一份实现；需要复用时抽为领域服务、纯函数、注册表或 Adapter 边界，并以测试防止旁路。
3. **六大设计原则作为准入门槛。** 修复和重构必须同时满足单一职责、开闭、里氏替换、接口隔离、依赖倒置和迪米特法则；如果当前代码结构导致无法一次修全同类问题，应优先重构边界，而不是复制补丁。
