...
2. 新增 `src/app/context-assembly-service.ts`，输出 `PromptContext`、`ContextSource[]`、`ContextBudgetReport`。
3. `extension.ts` 先调用新服务，旧 `project-rules.ts` 保留 fallback。

测试：

1. `project-instruction-service.test.mjs`
2. `context-assembly-service.test.mjs`
3. 现有 intent/workflow 回归。
...
1. 新增 `src/memory/memory-types.ts`、`memory-store.ts`、`sensitive-memory-guard.ts`。
