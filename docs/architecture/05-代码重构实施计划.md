...
3. **任务形态识别与既有工程集成原则。** 落代码前必须先判断当前任务属于既有大项目/正式项目内实现、独立编程任务、只读分析/设计/文档、验证修复续作或其他类型；如果是既有工程内新增功能，设计阶段必须先收集与原主控/平台/遥控器等整体架构的关系、职责边界、主入口、线程/事件模型、消息协议、数据结构、构建测试入口和可复用方法，再在这个背景下实现。涉及“参考既有通讯模块方式”的任务，必须从参考模块反向追踪项目级真实通讯链路，扩大到全项目通讯入口/出口、`uart*_tx/rx_main` 或等价通道、`TunnelTransport`/分片组装、MAVLink tunnel、publisher/subscriber、topic/payload_type/命令号、路由调度和主流程调用点，并把这些证据写入设计、接口和修改清单；禁止只在用户给出的目录内自洽实现。禁止把正式项目需求做成与主流程脱节的独立小模块、样例 `main` 或孤立测试程序；只有明确是独立任务、原型或练习时，才允许自建入口和自洽目录。
