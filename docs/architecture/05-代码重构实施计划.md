...
文档编号：ARCH-05
最后更新：2026-07-12
对应设计：[01-顶级编程智能体总体架构设计.md](01-顶级编程智能体总体架构设计.md)、[02-模型供应商与工具协议架构设计.md](02-模型供应商与工具协议架构设计.md)、[03-Agent运行时与工作流重构设计.md](03-Agent运行时与工作流重构设计.md)、[04-记忆体架构设计.md](04-记忆体架构设计.md)、[06-竞品实现方式对标审计与设计修正.md](06-竞品实现方式对标审计与设计修正.md)、[07-DeepSeek网页异常与恢复设计.md](07-DeepSeek网页异常与恢复设计.md)、[08-历史任务与续作架构设计.md](08-历史任务与续作架构设计.md)、[09-程序员工程完整性架构设计.md](09-程序员工程完整性架构设计.md)、[10-运行形态与界面解耦架构设计.md](10-运行形态与界面解耦架构设计.md)、[11-需求设计覆盖最终审计.md](11-需求设计覆盖最终审计.md)、[12-Agentic修复运行时专题设计.md](12-Agentic修复运行时专题设计.md)、[13-工程完整性与顶级增强核心设计.md](13-工程完整性与顶级增强核心设计.md)、[14-自动闭环迭代与测试方法论检讨.md](14-自动闭环迭代与测试方法论检讨.md)、[15-文件上下文与大文件治理专题设计.md](15-文件上下文与大文件治理专题设计.md)、[16-重复判定逻辑治理专题设计.md](16-重复判定逻辑治理专题设计.md)、[17-顶层RunContext与执行事实治理专题设计.md](17-顶层RunContext与执行事实治理专题设计.md)、[18-优秀编程智能体100%收敛与新窗口接管计划.md](18-优秀编程智能体100%收敛与新窗口接管计划.md)
状态：**superseded / historical implementation record**，不再是当前迭代路线或完成状态权威
建议开发分支：`devseek-multi`

> 执行权已收口：本文 Phase 0～12、“已实施”和历史验证数字只保留为演进证据，不等于当前 `wired` 或 `qualified`，也不得继续按 Phase 领取任务。当前任务只由 [PLAN](../top-agent-convergence-audit-20260711/PLAN-当前收敛迭代计划.md) 发放，active architecture 由 [`devseek-active-baseline-selector.json`](../process/devseek-active-baseline-selector.json) 选择；历史 backlog、接管协议与承接裁决分别保存在 [14](../top-agent-convergence-audit-20260711/archive/14-未完成事项与后续整体迭代计划.md)、[15](../top-agent-convergence-audit-20260711/archive/15-新窗口与跨模型接管手册.md) 和 [18](../top-agent-convergence-audit-20260711/archive/18-旧架构实施计划承接矩阵与GPT5.5唯一执行基线.md)。
...
### 2.3 真实运行与收敛审计（2026-07-11）

本轮修改前再次对标 Claude Code/Codex：成熟编程智能体使用结构化工具调用或宿主文件写入边界传递源码，不要求模型把整份多行源码手工转义成 JSON；确定性回归、真实 Provider 冒烟、真实插件闭环是不同资格等级，任何一级绿灯都不能替代更高一级；“稳定”必须绑定当前运行版本和可回放证据，而不是凭 UI、模型文字或旧 VSIX 判断。

事实结论：

1. 2026-07-11 保留的 7 份 `devseek-real-plugin-deepseek` 正式仿真报告全部为 `ok:false`。失败从“短意图/截断/工具未执行”逐步收敛到“工具转录损坏、重复全文件写入、终端修复无进展、缺少最终收敛”，说明修复在推进，但尚无证据证明复杂正式任务稳定完成。
2. 截至提交 `56effd7`，Extension 108 个 suite、Bridge 20 个测试和 PA0-PA13 确定性 benchmark 已通过；这些结果只证明回归与运行时机械链路，不证明 DeepSeek Web 模型交互和真实 VS Code 插件稳定。
3. 最近正式任务暴露了两项通用根因：DeepSeek “继续生成”可能从头重启，导致同一路径重复全量写入；多行源码经手写 JSON 文本工具传输时，`\n` 等字面量可能被破坏。前者已由 continuation merger、重复写入 replay 和最终写入保留策略治理；后者统一改为嵌套 XML + CDATA 的无损 mutation 协议。
4. 产物质量仍存在项目事实不足和臆造协议值风险。正式工程任务必须以源码证据矩阵、项目级入口/链路、接口 schema、原代码修改清单和真实验证结果结算，不能只看文件是否生成。
5. Headless Agent Core 尚未完全成为唯一事实源；7 个历史编排/兼容/Surface 文件仍超过目标规模。新增 `devseek-architecture-budgets.json` 冻结当前上限，后续职责只能外移、上限只能下降，禁止继续向超大文件堆修复。
6. 首个 L3 短 canary 还暴露出任务形态与测试 oracle 的共同缺陷：请求“读取源码事实、创建报告、不要修改源码”时，旧路由把末尾只读约束扩大成整任务只读并直接返回文件存在性；旧仿真器又把维保正式项目关键词和修改清单要求硬编码到所有 Markdown。现统一按子句区分输入只读约束与输出交付动作，仿真质量规则改为 canary/medium/formal profile + 当前任务必含事实，不再绑定某个测试业务。

统一资格等级：

1. **L0 单元/协议**：状态机、工具解析、Provider 完整性、写盘和质量判定的主链路与失败链路。
2. **L1 Runtime Replay**：历史真实日志必须保留真实失败，同时消除已修复的误报；每个新失败类别先固化最小 fixture。
3. **L2 确定性仿真**：Extension 全量单测、Bridge、PA benchmark 和 scripted Provider 故障矩阵通过，只能标记 `deterministic`。
4. **L3 真实短任务**：同一 Git/VSIX 下完成只读事实提取、微小代码修改、包含反斜杠/代码围栏的 Markdown 写入；报告必须通过 Runtime Replay。
5. **L4 中型工程任务**：需求调查、跨目录设计、代码修改和项目验证形成闭环。
6. **L5 正式项目任务**：使用用户原始复杂需求，成果物质量与 Codex 参考实现逐项对标。

收敛和停止规则：

1. 同一失败类别连续出现 2 次时，不允许第 3 次盲目真实重跑；必须提取 fixture、定位责任边界并先通过 L0-L2。
2. 每轮只改变一个主要架构变量；通过 benchmark 后立即提交，真实报告必须绑定该提交的运行日志 `gitCommit`，dirty worktree 不得借用 `HEAD` 身份取得候选或稳定资格。
3. 单次真实成功只能标记候选可用。只有同一提交连续取得 3 个短任务、2 个中型任务、1 个正式任务成功，且均无错误级 replay issue，才允许标记 `stable`。
4. 真实长任务前先跑短 canary；canary 失败时禁止消耗时间继续正式任务。
5. 每轮同时评估运行日志和成果物；文件存在、todo 变绿或模型声称完成都不是交付证据。
6. 仿真 oracle 必须由任务形态和显式成果物断言驱动；禁止把当前维保、license 或固定文档数量写成通用通过条件。

### 2.4 Exact Grounding 与副作用提交边界收口（2026-07-11）

对标 Claude Code/Codex 后的职责结论是：模型负责调查、推理和普通候选，宿主负责授权、可信证据、exact bytes、写入原子性、read-back、回滚及最终交付真相。模型自称“已写入”或 UI 状态不得成为磁盘事实；逐字结构已经完全可执行时也不应继续让 Provider 重写。

本轮 `7981f99 -> e8d36f4 -> 368cacf` 已完成：

1. `EvidenceRef -> ArtifactClaim -> VerificationResult` 进入源码事实报告和 RunContext/Replay 完成绑定，证据不足、claim 不一致或 read-back 漂移均禁止完成。
2. 完整 exact Markdown 契约由宿主从独立源码证据物化，保留源码初始化器表达；普通 Provider 候选最多一次内存修复，只有最终候选通过写前结构、质量和 grounding 才提交一次。
3. Markdown executor 在任务开始捕获目标 canonical route、最近既有父目录身份与内容基线；授权等待后使用 CAS、同目录 temp、payload fsync、atomic rename 和 commit token 提交。
4. 提交后重新读取目标与源文件；callback/status 失败不能触发重复写，回滚只在 route/inode/content identity 仍匹配 commit token 时执行。
5. 中英文 exact 契约、否定、局部限制、后续更正、目标/源码漂移、父目录 symlink swap、callback/status 异常、权限/模式和失败临时文件清理均有回归。
6. 干净功能提交 `368cacf` 通过 Extension 113/113 suites 和 Phase 0-12 九项门禁；精确包已校验安装。但最近真实 canary 仍是 `e8d36f4` 的安全失败，`368cacf` 未运行 live，资格保持 `0/3、0/2、0/1`。

下一轮架构工作不是再为 Markdown 添加补丁，而是把同一提交抽象迁移到所有 sibling mutation：`simple-file-task`、`deterministic-task-executor`、`tool-loop`、legacy `agent-loop`、`workspace-applier`、Pending Edit Undo/Hunk Undo；同时删除 `auto-validation` 的隐藏写盘，封存 Agent 路径 legacy write API，补 direct-fs 静态守卫、跨平台 anchored-directory 策略和 ENOSPC/rename/fsync/symlink/ABA 故障注入。完整顺序和退出条件以 ARCH-18 的 P0-MUTATION 为准。
...
| ArchitectureDrift | 超目标编排/Surface 文件被冻结；职责外移后预算上限只降不升 |
| ProviderTransport | 多行源码/Markdown 无损传输；截断、续写重启、重复写入均可识别和回放 |
| LiveQualification | 确定性、真实 Provider、真实插件分级；报告绑定当前 Git/VSIX，不得用旧版本证明新版本 |
| ArtifactGrounding | 正式项目产物含源码事实矩阵、项目级链路、接口契约、修改清单和验证证据，无臆造常量 |
...
7. 新失败类别已固化为最小单测或 Runtime Replay fixture，并验证同类入口。
8. 确定性门禁与真实运行资格分开记录，不得把 benchmark 通过表述为产品稳定。
9. 真实插件报告绑定当前提交；需要宣称稳定时满足 3 个短任务、2 个中型任务、1 个正式任务的同提交证据配额。
10. 每轮审查生成文档、代码和验证产物的工程质量，而不只审查日志或 UI 状态。
