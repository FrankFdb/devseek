...
15. **同任务参考实施与质量对标原则。** 每次正式项目能力测试必须允许用同一用户任务生成一份 Codex/Claude Code 风格的参考实施或参考分析，再把 DevSeek 产物与参考产物逐项对比：原项目调查深度、具体常量/数值/协议字段、接口文档完整度、原代码修改清单、代码集成边界、验证证据和用户可读性。对标结论必须进入测试报告，不能只看 `completed` 或文件存在。
16. **测试产物隔离原则。** 真实项目测试不得把仿真文档和代码散落到正式源码目录；每次测试必须在目标模块下创建到分钟级时间戳目录，例如 `202607101055/`，其下固定分为 `docs/` 和 `src/`，文件名使用正式业务命名，不添加 `selfloop`、`codex`、`verify` 等临时后缀。需要修改正式原代码时，测试产物先在文档中列出“原有代码修改清单”，明确文件、函数/类、改动内容、原因、风险和验证方式。
