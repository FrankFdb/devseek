...
3. **顶级编程智能体目标必须持续校准。** 每次迭代都要推动 DevSeek 在任务理解、工具边界、证据闭环、历史可追溯和用户可控性上追平甚至超越 Claude Code/Codex；不得用局部补丁掩盖错误边界。
4. **先建立边界，再迁移职责，最后增强能力。** 不做“大爆炸重写”，但目标态必须是革命性的 Headless Agent Core / Agent Runtime；每一阶段都必须能独立测试、独立回退，多入口重构在 `devseek-multi` 分支推进，main 只接收阶段完成、测试通过、可回退的合并。
5. **入口和 Surface 只能做适配。** `extension.ts`、CLI main、Desktop main 等入口只能注册 surface、注入依赖、启动服务和做生命周期清理；VS Code WebView、命令、Inline Chat、CLI TUI、JSONL、Desktop/local Web 都只能把用户输入转换为 `AgentCommand`，把 `AgentEvent` 渲染为界面；同一任务事实必须由 Headless Agent Core 产生，不能复制 Agent 业务判断。
6. **权限、审计和证据闭环是新增路径准入条件。** 任何写盘、终端、网络、MCP、VS Code command 的新增路径，都必须先接入权限、审计、EvidenceRef、Checkpoint、ReviewLedger 和 QualityGate；模型 prose、UI 状态或半截 Provider 输出不能绕过事实层结算任务。
7. **同类型问题必须系统性修复。** 任何 bugfix、优化或重构都不能只修当前复现路径；必须审计同类入口、状态流、消息/工具/权限/验证边界和历史恢复路径。若存在重复逻辑或多处相同判断，必须收敛为单一抽象、单一出口或单一领域服务，并补上覆盖同类路径的测试与静态守卫；除非有明确架构差异，完全一样的逻辑在项目中原则上只能有一份。
8. **六大设计原则是修复准入条件。** 每次修改前后都要用单一职责、开闭、里氏替换、接口隔离、依赖倒置和迪米特法则自检；发现复制逻辑、跨层调用、UI/Provider/Agent/Workspace 互相知道过多、或业务层依赖具体基础设施时，应先重构边界再落修复，不得把局部补丁当成最终方案。
9. **代码规模必须服务 AI 可维护性。** 行数阈值只是审计触发器，不是重构目标：生产代码文件超过 1024 行必须审计职责，复杂生产模块超过 2048 行必须制定拆分计划，超过 3000 行必须作为重构优先级处理；普通函数控制在 100 行以内，复杂编排函数控制在 300 行以内。拆分必须让职责更单一、依赖方向更正确、后续 Claude Code/Codex 类能力更容易接入。
10. **文件上下文和工具协议必须词典化治理。** 大文件读取不得隐式截断；必须通过统一文件上下文服务返回总行数、返回范围、截断状态和续读方式。工具名、工具别名、参数别名、权限分类、执行路径和 UI 清理必须以 `tool-registry` 为单一事实源，并用测试守卫前后端镜像同步，避免模型输出新别名时只显示不执行。
11. **修改 Extension 或 Bridge 行为后必须执行本地发布闭环。** 按仓库要求完成 compile、package、install VSIX，并确认 VS Code 当前运行版本与本次构建一致。
12. **分层闭环测试准入原则：用户插件实测不能作为主要缺陷发现手段。** 任何 Agent/Provider/工具/文件交付修复都必须先建立 `unit 测试 -> run-log replay 测试 -> 仿真 DevSeek Surface + DeepSeek Web 交互测试 -> 真实 DevSeek 插件 + 真实 DeepSeek Web live 抽查` 的分层门禁；unit 守纯策略和边界，replay 固化真实失败日志，仿真交互守同一用户场景下的 Provider 往返、工具证据、文件落盘、QualityGate 和最终交付。真实插件 live 测试因依赖账号、网页和人工登录，只能作为显式 opt-in 发布/回归抽查，但凡改动 VS Code Extension、Bridge、Provider 网页交互、文件交付或状态结算，必须至少提供可运行的 live harness，并在需要真实网页确认时执行；live 失败必须先沉淀为 replay 或仿真 fixture，再修复和回归，避免把人工插件截图测试变成反复试错主流程。
