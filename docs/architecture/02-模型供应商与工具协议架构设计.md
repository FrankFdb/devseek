---
devseek_governance:
  generator: "devseek-doc-governance/v1"
  status: "historical"
  path: "docs/architecture/02-模型供应商与工具协议架构设计.md"
  source_group: "architecture"
  decision: "keep"
  relationship: "legacy-architecture"
  active_baselines:
    - "docs/architecture/01-顶级编程智能体总体架构设计.md"
  machine_sources:
    active_selector: "docs/process/devseek-active-baseline-selector.json"
    legacy_inventory: "docs/process/devseek-legacy-doc-inventory.json"
  asserts_gate_pass: false
---

<!-- DEVSEEK-GOVERNANCE-BANNER:START -->
> [!NOTE]
> DevSeek governance: this document is `historical` with decision `keep` and relationship `legacy-architecture`. Current authority: `docs/architecture/01-顶级编程智能体总体架构设计.md`. Machine source: `docs/process/devseek-legacy-doc-inventory.json`.
<!-- DEVSEEK-GOVERNANCE-BANNER:END -->

# DevSeek 模型供应商与工具协议架构设计

文档编号：ARCH-02
最后更新：2026-07-02
对应需求：[../requirements/02-顶级编程智能体需求基线.md](../requirements/02-顶级编程智能体需求基线.md)、[../requirements/07-架构重构需求澄清.md](../requirements/07-架构重构需求澄清.md)
状态：活跃模型与工具协议设计

## 1. 设计目标

本设计不再把 Provider 当成“换模型的 HTTP 客户端”。顶级编程智能体需要的是统一的模型能力契约和工具调用协议：不同 Provider 可以有不同能力，但进入 DevSeek Agent Runtime 后必须被规范化为同一套消息、工具调用、权限和证据结构。

目标：

1. DeepSeek Web Bridge 作为默认 Provider，保留低门槛网页路径。
2. 支持 API Provider 直接接入 DeepSeek API、OpenAI-compatible、本地模型和企业私有模型。
3. 支持 API Provider 的原生 function calling / structured output。
4. 支持 VS Code LM、OpenAI compatible、企业私有 Provider。
5. 把工具调用统一转换为 `ToolCall`，再交给 `ToolRegistry`、`PermissionKernel`、`ToolExecutor`。
6. Provider 失败时可健康检查、降级、重试和回退，不污染工作流状态。
7. DeepSeek Web 响应必须经过完整性检测；截断、重复、未闭合工具块不能进入执行链。
8. Provider 恢复只恢复模型会话，不重复执行已完成的副作用工具。
9. 文本工具方言必须先进入 `ModelToolProtocolAdapter` 适配层，再转换为内部工具对象；执行器、UI、完整性检查不得各自维护协议正则。

## 2. 能力模型

```typescript
export interface LLMProviderCapabilities {
  streaming: boolean;
  systemPrompt: boolean;
  nativeToolCalling: boolean;
  structuredOutput: boolean;
  toolResultMessages: boolean;
  reasoningEffort?: boolean;
  vision?: boolean;
  maxInputTokens?: number;
  maxOutputTokens?: number;
  resumableSession?: boolean;
  responseIntegritySignals?: boolean;
}

export interface LLMProvider {
  id: string;
  displayName: string;
  capabilities: LLMProviderCapabilities;
  health(): Promise<ProviderHealth>;
  stream(request: LLMRequest): AsyncIterable<LLMEvent>;
}

export interface LLMProviderConfig {
  id: string;
  kind: 'deepseek-web' | 'api' | 'openai-compatible' | 'vscode-lm' | 'local' | 'enterprise';
  enabled: boolean;
  priority: number;
  model?: string;
  baseUrl?: string;
  secretRef?: string;
  defaultFor?: WorkflowMode[];
}
```

设计约束：

1. Workflow 不判断 Provider 细节，只读取能力和统一事件。
2. Provider 不执行工具，只产生候选工具调用。
3. DeepSeek Web 的 `[TOOL:name {...}]` 是兼容输入，不是 Agent 内部协议。
4. 工具调用参数必须经过 schema 校验和权限判定。
5. API key、cookie、token 只能通过 `secretRef` 引用，不能进入 prompt、任务历史、记忆或日志。
6. `Action / Action Input`、`Calling`、DSML、XML、JSON 数组、function-style 等模型输出格式都属于 Provider 方言，必须通过同一个 dialect registry 归一化。
7. ReAct 方言必须兼容真实 DeepSeek Web 的黏连输出，例如 `Action: read_fileAction Input: {"path":"..."}`；协议适配层不能依赖换行、空格或 Markdown 格式存在。
8. 新增模型方言时必须同时提供 parse、findStart、strip 三类 contract test，证明可执行、可判定、可从用户可见文本移除。
9. 模型响应中出现 `[工具执行结果]`、`[工具返回]`、`[read_file:]` 等工具结果边界时，`ModelToolProtocolAdapter` 必须只解析边界之前的工具请求区；边界之后的内容一律视为 provider-authored transcript，不得进入工具执行、命令统计或 UI 工具协议解析。

## 3. 默认 Provider 与 API 接入策略

默认策略：

1. 首次启动和未配置模型时，`ProviderSelector` 选择 `DeepSeekWebProvider`。
2. 用户显式配置 API Provider 后，`ProviderConfigService` 记录 provider id、base url、model、capabilities 和 secret 引用。
3. API Provider 可以直接接入 DeepSeek API、OpenAI-compatible、本地模型、企业网关；接入后仍只实现 `LLMProvider`，不能改 Agent Runtime。
4. Provider 切换只影响模型请求和能力降级，不影响 `ToolRegistry`、`PermissionKernel`、`QualityGateService`、`TaskHistoryStore`、`MemoryService`。
5. DeepSeek Web 不可用时，可提示切换 API Provider；fallback 必须继承同一 workflow id、checkpoint、ReviewLedger 和 idempotency ledger。
6. 低能力 Provider 不能绕过功能缺口，例如无原生工具调用时必须走文本工具解析和完整性检查，无 structured output 时必须标低置信并进入校验。

## 4. 架构图

```mermaid
flowchart TB
  Runtime[AgentRuntime] --> Router[LLMProviderRouter]
  Runtime --> Config[ProviderConfigService]
  Router --> Selector[ProviderSelector]
  Router --> Health[ProviderHealthMonitor]
  Router --> Adapter[PromptAdapter]
  Router --> Recovery[ProviderRecoveryService]

  Config --> Selector
  Selector --> WebProvider[DeepSeekWebProvider default]
  Selector --> ApiProvider[ApiProvider]
  Selector --> VSCodeProvider[VSCodeLMProvider]
  Selector --> CompatibleProvider[OpenAICompatibleProvider]

  WebProvider --> Bridge[BridgeClient]
  ApiProvider --> Http[HTTP Client]
  VSCodeProvider --> VSCodeLM[VS Code LM API]
  CompatibleProvider --> Http

  WebProvider --> ProtocolAdapter[ModelToolProtocolAdapter]
  WebProvider --> Integrity[ResponseIntegrityChecker]
  ApiProvider --> NativeTools[NativeToolCallAdapter]
  VSCodeProvider --> NativeTools
  CompatibleProvider --> NativeTools

  Integrity --> ProtocolAdapter
  ProtocolAdapter --> Normalizer[ToolCallNormalizer]
  NativeTools --> Normalizer
  Normalizer --> Registry[ToolRegistry]
  Registry --> Permission[PermissionKernel]
  Permission --> Executor[ToolExecutor]
  Recovery --> Router
```

## 5. Provider 状态图

```mermaid
stateDiagram-v2
  [*] --> Unknown
  Unknown --> Healthy: health ok
  Unknown --> Degraded: partial capability
  Unknown --> Unavailable: health failed
  Healthy --> Streaming: request started
  Streaming --> Healthy: completed
  Streaming --> Degraded: recoverable error
  Streaming --> Interrupted: timeout/truncated/bridge lost
  Streaming --> Unavailable: repeated failure
  Interrupted --> Rehydrating: checkpoint exists
  Rehydrating --> Streaming: resume prompt accepted
  Rehydrating --> Degraded: partial recovery
  Degraded --> Healthy: health restored
  Degraded --> FallbackSelected: better provider available
  Unavailable --> FallbackSelected: fallback policy
  FallbackSelected --> Healthy: fallback ok
```

## 6. 请求时序图

```mermaid
sequenceDiagram
  participant A as AgentRuntime
  participant R as ProviderRouter
  participant C as ProviderConfigService
  participant P as Provider
  participant I as ResponseIntegrityChecker
  participant N as ToolCallNormalizer
  participant TR as ToolRegistry
  participant PK as PermissionKernel
  participant TX as ToolExecutor

  A->>R: stream(LLMRequest)
  R->>C: load default and enabled providers
  R->>R: select provider by capability, health and user choice
  R->>P: stream(adapted request)
  P-->>R: text delta / native tool event
  R->>I: inspect completeness and stability
  I-->>R: complete / partial / corrupted
  R->>N: normalize complete provider event
  N-->>A: assistant delta
  N->>TR: resolve ToolCall
  TR->>PK: authorize ToolCall
  PK-->>TR: allow / confirm / deny
  TR->>TX: execute allowed call
  TX-->>A: ToolResult with evidence
  A->>R: continue with tool result message
```

## 7. 类图

```mermaid
classDiagram
  class LLMProviderRouter {
    +select(request)
    +stream(request)
  }
  class ProviderConfigService {
    +getDefaultProvider()
    +listEnabled()
    +resolveSecretRef()
  }
  class LLMProvider {
    <<interface>>
    +capabilities
    +health()
    +stream(request)
  }
  class ProviderSelector {
    +rank(request, providers)
  }
  class PromptAdapter {
    +adapt(request, provider)
  }
  class ResponseIntegrityChecker {
    +inspect(event)
    +isExecutable()
  }
  class ProviderRecoveryService {
    +recover(providerId, checkpoint)
    +buildResumeRequest()
  }
  class ToolCallNormalizer {
    +fromNative(event)
    +fromText(delta)
  }
  class ModelToolProtocolAdapter {
    +parse(text)
    +findStart(text)
    +strip(text)
    +isolateToolRequest(text)
  }
  class ToolRegistry {
    +resolve(name)
    +validate(call)
  }
  class PermissionKernel {
    +authorize(call, policy)
  }
  class ToolExecutor {
    +execute(call)
  }

  LLMProviderRouter --> ProviderSelector
  LLMProviderRouter --> ProviderConfigService
  LLMProviderRouter --> PromptAdapter
  LLMProviderRouter --> LLMProvider
  LLMProviderRouter --> ResponseIntegrityChecker
  LLMProviderRouter --> ProviderRecoveryService
  LLMProviderRouter --> ModelToolProtocolAdapter
  ResponseIntegrityChecker --> ModelToolProtocolAdapter
  ModelToolProtocolAdapter --> ToolCallNormalizer
  LLMProviderRouter --> ToolCallNormalizer
  ToolCallNormalizer --> ToolRegistry
  ToolRegistry --> PermissionKernel
  PermissionKernel --> ToolExecutor
```

## 8. 工具协议

```typescript
export interface ToolDefinition {
  name: string;
  kind: 'read' | 'search' | 'edit' | 'terminal' | 'network' | 'mcp' | 'memory' | 'vscode' | 'plan';
  description: string;
  inputSchema: JsonSchema;
  outputSchema: JsonSchema;
  mutatesWorkspace: boolean;
  requiresConfirmation: boolean;
  riskLevel: 'low' | 'medium' | 'high' | 'critical';
  allowedInModes: WorkflowMode[];
}

export interface ToolCall {
  id: string;
  name: string;
  input: unknown;
  origin: 'native-tool' | 'text-tool' | 'user-action' | 'hook';
  providerId?: string;
  operationId: string;
  replayPolicy: 'never' | 'read-only' | 'requires-confirmation';
}

export interface ToolResult {
  id: string;
  toolName: string;
  ok: boolean;
  structured?: unknown;
  summary: string;
  evidence: EvidenceRef[];
  error?: ToolError;
}
```

### 8.1 模型工具协议适配层

`ModelToolProtocolAdapter` 是所有文本工具方言的唯一入口。它的职责不是执行工具，而是把不同模型或网页桥接返回的方言统一转换成内部工具候选：

1. `parse(text)`：从模型文本中解析出候选工具调用，返回统一 `ToolCall`/伪工具对象。
2. `findStart(text)`：判断流式文本中第一个工具协议起点，用于截断用户可见 prose、检测 incomplete block 和阻止协议文本进入 shell。
3. `strip(text)`：从用户可见文本、final summary、history payload 中移除内部工具协议。

执行型工作区请求不能依赖 Provider 在普通正文里贴完整源码完成任务。若模型输出源码代码块但没有结构化工具调用、文件应用证据或本地终端验证证据，运行日志 replay 必须把它诊断为 `source-output-without-application`，由路由或应用层修复，而不能让 WebView 把源码展示当成任务完成。

已纳入同一 registry 的方言包括：

1. `[TOOL:name {...}]` 兼容格式。
2. Markdown/DeepSeek `Calling`、`Tool: ... Arguments:`、shell transcript。
3. DSML、全角 DSML、XML self-closing / paired tags。
4. `<TOOL_CALL>...</TOOL_CALL>` envelope。
5. function-style，例如 `read_file({...})`、`search_content({...})`。
6. JSON 工具数组和 task summary JSON。
7. ReAct 风格 `Action: tool` + `Action Input: {...}`。

新增方言的规则：

1. 只能新增一个 dialect，不得在 `ToolExecutor`、terminal guard、WebView sanitizer、ResponseIntegrityChecker 中分散补正则。
2. 同一 dialect 必须覆盖 parse、findStart、strip 三条路径。
3. 必须有 parser contract test、WebView outbound test、terminal guard 或 integrity test 中至少两类回归。
4. 若方言存在流式 split 场景，必须测试半段和完整段，避免界面闪现协议文本。

## 9. Provider 能力降级

| Provider 能力 | 优先实现 | 降级路径 |
| --- | --- | --- |
| 原生工具调用 | 直接输出 `ToolCall` | 文本工具解析后规范化 |
| structured output | JSON schema 校验 | 从文本提取计划并标低置信 |
| system prompt | 独立 system message | 合并到首条开发者上下文 |
| tool result message | role=tool 回传 | 摘要化为 assistant/user 上下文 |
| streaming | 增量事件 | 非 streaming Provider 输出完整响应 |
| 大上下文 | 完整 ContextAssembly | 预算裁剪和来源报告 |
| 响应完整性信号 | Provider 原生 finish reason / tool status | `ResponseIntegrityChecker` 基于文本块、工具闭合、停止原因和超时判断 |
| 可恢复会话 | Provider session id / thread id | `TaskCheckpointStore` 重建恢复 prompt |
| 默认可用性 | DeepSeek Web 默认 Provider | 用户配置的 API Provider 接管同一 workflow 和 checkpoint |
| API secret | VS Code SecretStorage / OS keychain | 禁止落入任务历史、记忆、prompt 和普通日志 |

## 10. 安全边界

1. Provider 无权决定是否执行工具。
2. Provider 无权绕过 protected files、workspace boundary、network confirmation。
3. Provider 失败不能直接重试破坏性工具。
4. 任何 fallback 都必须保留同一 workflow id 和审计事件。
5. DeepSeek Web Bridge 的 DOM/Playwright 变化只影响 `DeepSeekWebProvider`，不能泄漏到 Agent Runtime。
6. 响应截断、重复、半段 diff、半段工具调用必须进入 blocked 状态，不能被 normalizer 执行。
7. 任何 Provider retry 都必须经过 `IdempotencyGuard`，避免重复写盘、重复执行终端、重复调用 destructive MCP。
8. API Provider 的 `apiKey`、cookie、session token 只能保存在 SecretStorage 或系统密钥链，Provider transcript 只能保存摘要和 hash。

## 11. 迁移路径

1. 保留当前 `llm/provider-router.ts` 作为入口，扩展 capabilities 和 health。
2. 新增 `ProviderConfigService`，默认选择 DeepSeek Web，并支持 API Provider 配置、secret 引用和健康状态。
3. 把 `agent-loop.ts` 中的文本伪工具解析迁入 `ModelToolProtocolAdapter`，再由 `ToolCallNormalizer` 规范化。
4. 把 Provider 响应统一转换为 `LLMEvent`，再由 Runtime 消费。
5. 给 DeepSeek Web、DeepSeek API、OpenAI-compatible、本地 API Provider、VS Code LM 写同一套 contract tests。
6. 在所有工具调用进入执行器前，强制经过 `ToolRegistry.validate()` 和 `PermissionKernel.authorize()`。
7. DeepSeek Web Bridge 先增加 `ResponseIntegrityChecker` 和 `ProviderRecoveryService`，再开放自动恢复。
8. 工具执行先增加 `operationId` 和 replay policy，再允许 Provider retry。

## 12. 验收标准

1. 无用户配置时，默认 Provider 是 DeepSeek Web Bridge。
2. 配置 API Provider 后，可使用 base url、model 和 secret 引用接入同一 Agent Runtime。
3. DeepSeek Web 文本工具和 API native tool calling 都规范化为同一种 `ToolCall`。
4. Provider 切换、fallback 或恢复不会重复执行已提交副作用。
5. Provider secret 不出现在 prompt、任务历史、记忆、ReviewLedger 或普通日志中。
6. 任一模型工具方言在执行、UI 展示、final summary、history 恢复和 terminal guard 中得到一致结果。
