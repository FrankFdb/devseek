...
devseek_governance:
  ...
  status: "historical"
  path: "docs/architecture/10-运行形态与界面解耦架构设计.md"
...
| Package | 当前状态 | 后续目标 |
| --- | --- | --- |
| `packages/shared` | 已启动 app core | 承载 schema、协议、纯类型、AgentApplicationService、Surface/Platform/BuildProfile 基础件 |
...
| `packages/cli` | 已启动 | 承载 CLI TUI 最小交互、`devseek exec --jsonl`、shell/stdio 入口 |
...
## 9.1 Phase 10 当前实现状态（2026-06-21）

| 项目 | 状态 | 说明 |
| --- | --- | --- |
| `AgentCommand` / `AgentEvent` | 已启动并抽入 shared | `packages/shared/src/agent-protocol.ts` 定义应用级命令/事件、Surface capability 和 Platform profile；后续继续把 workflow、review、validation 事件从 UI 回调迁入协议 |
| `AgentApplicationService` | 已启动并抽入 shared | `routeChat` 的 Provider 选择、bridge/API 路由、vision payload、API Key 恢复和历史记录端口已迁入 `packages/shared/src/agent-application-service.ts` |
| VS Code composition root | 继续收敛 | `extension.ts` 只保留 `routeChat` 薄委托和依赖注入；`runChat` 的 session turn lifecycle 已迁入 `ChatSessionTurnService`，剩余 agent loop 事实继续拆分 |
| Surface Adapter | 已启动 | `SurfaceAdapter` 和 capability presets 位于 shared；`VSCodeSurfaceAdapter` 已把 WebView chat 输入转换为 `chat.request` command |
| Platform Runtime | 已启动 | 已落地 PlatformRuntimeAdapter、ShellAdapter、PathAdapter、StorageAdapter、BrowserBridgeAdapter 抽象和 POSIX/Windows/WSL profile 检测 |
| CLI / JSONL | 已启动 | `packages/cli` 提供 `devseek exec`、`--jsonl`、`--mock`、交互 prompt、history/resume 和 bridge chat port |
| BuildProfile | 已启动 | `BUILD_PROFILES` 覆盖 shared、bridge、VS Code extension、CLI TUI、CLI JSONL、future desktop/local web |

Phase 10 当前目标是建立正确边界并提供最小可验证多入口，而不是复制 VS Code agent loop。下一步固定顺序为：checkpoint/task facts 迁入 app stores -> ReviewLedger/QualityGate/pending edit 事件化 -> CLI 复用同一 workflow facts -> Desktop/local Web 评估。
