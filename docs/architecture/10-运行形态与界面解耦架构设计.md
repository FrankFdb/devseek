...
| VS Code composition root | 部分完成 | `extension.ts` 只保留 `routeChat` 薄委托和依赖注入；`runChat` 仍是下一步要迁移的 workflow use case |
...
Phase 10 当前目标是建立正确边界并提供最小可验证多入口，而不是复制 VS Code agent loop。下一步固定顺序为：`runChat` 拆成 workflow use case -> session/checkpoint/task facts 迁入 app stores -> ReviewLedger/QualityGate/pending edit 事件化 -> CLI 复用同一 workflow facts -> Desktop/local Web 评估。
