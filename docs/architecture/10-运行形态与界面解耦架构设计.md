...
## 9.1 Phase 10a 当前实现状态（2026-06-21）

| 项目 | 状态 | 说明 |
| --- | --- | --- |
| `AgentCommand` / `AgentEvent` | 已启动 | `app/agent-protocol.ts` 已定义应用级命令/事件、Surface capability 和 Platform profile；后续继续把 workflow、review、validation 事件从 UI 回调迁入协议 |
| `AgentApplicationService` | 已启动 | `routeChat` 的 Provider 选择、bridge/API 路由、vision payload、API Key 恢复和历史记录端口已迁入 `app/agent-application-service.ts` |
| VS Code composition root | 部分完成 | `extension.ts` 只保留 `routeChat` 薄委托和依赖注入；`runChat` 仍是下一步要迁移的 workflow use case |
| Surface Adapter | 部分完成 | WebView provider 与 command registration 已拆到 `ui/*`，但仍需通过 `SurfaceAdapter` 完整接入 `AgentCommand` / `AgentEvent` |
| Platform Runtime / CLI / JSONL | 未开始 | 必须等 `runChat` 和任务事实迁入 Agent Core 后再实现，避免新入口复制 VS Code 逻辑 |

Phase 10a 的目标是建立正确边界，而不是宣布多入口完成。下一步固定顺序为：`runChat` 拆成 workflow use case -> session/checkpoint/task facts 迁入 app stores -> WebView callbacks 收敛成 SurfaceAdapter -> PlatformRuntimeAdapter -> CLI/JSONL。
