...
test('DeepSeekAgent: incomplete action cues keep generation open', () => {
  const agent = src('src/deepseek-agent.ts');
  const progress = src('src/deepseek-response-progress.ts');

  assert.match(agent, /looksLikeIncompleteAssistantIntent/);
  assert.match(progress, /function looksLikeIncompleteAssistantIntent/);
...
test('DeepSeekAgent: reused-container visible text diff must be substantive before response starts', () => {
  ...
  assert.match(progress, /function isSubstantiveAssistantTextDiff/);
  assert.match(progress, /looksLikeSubstantiveAssistantText/);
  assert.match(progress, /read_file\|grep_search\|list_dir/);
  assert.match(progress, /trimmed\.length >= 120/);
  assert.match(progress, /trimmed\.length >= 80/);
...
test('DeepSeekAgent: streaming has an absolute wall-clock timeout', () => {
  ...
  assert.match(progress, /RESPONSE_ABSOLUTE_TIMEOUT_MAX_MS = 180_000/);
  assert.match(agent, /responseAbsoluteTimeoutMs/);
  assert.match(agent, /streaming-absolute-deadline/);
  assert.match(progress, /RESPONSE_CORRUPTED:stream-timeout/);
