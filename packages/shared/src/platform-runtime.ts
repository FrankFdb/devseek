...
export function assertPlatformRuntimeProfileSupported(profile: PlatformProfile): void {
  ...
  if (!evaluation.supported) throw new PlatformRuntimeProfileError(evaluation);
}
...
function supportedProfile(kind: PlatformAdapterProfileKind, profile: string): PlatformAdapterProfile {
...
function formatUnsupportedAdapters(unsupported: readonly PlatformAdapterProfile[]): string {
  return unsupported.map(adapter => `${adapter.kind}=${adapter.profile}`).join(', ');
}
