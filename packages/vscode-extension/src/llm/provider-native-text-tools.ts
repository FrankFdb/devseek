...
export function projectProviderNativeTextToolResponse(
  ...
  return hasCalling
    ? projectCallingResponse(text)
    : projectToolArgumentsResponse(text);
}
...
function projectToolArgumentsResponse(text: string): ProviderNativeTextToolResponse | undefined {
  const tools: FakeTool[] = [];
  const header = new RegExp(TOOL_ARGUMENTS_HEADER_RE.source, TOOL_ARGUMENTS_HEADER_RE.flags);
  let cursor = 0;
  let prose = '';
