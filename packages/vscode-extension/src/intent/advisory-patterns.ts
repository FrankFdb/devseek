...
const ADVISORY_PLAN_RE = /(?:(?:给出|提供|输出|列出|制定|梳理).{0,40}(?:建议|对策|检讨|方案|计划|任务|task|步骤|清单)|(?:建议|对策|检讨).{0,30}(?:分析|方案|计划|任务|task)|(?:从|站在).{0,24}(?:角度|视角).{0,24}(?:给出|输出|列出).{0,20}(?:task|任务|建议|对策)|(?:suggest|recommend|propose|list|outline|draft).{0,60}(?:ways?|options?|recommendations?|plan|steps?|approach|how\s+to|fix|repair|refactor)|(?:how\s+to|ways?\s+to).{0,40}(?:fix|repair|refactor|improve|change|update))/i;
...
const DELIVERABLE_WRITE_RE = /(?:(?:新增|创建|新建|生成|编写|写入|输出|保存|落盘|提供|重命名|改名|移动|移到|挪到|挪动|复制|拷贝|追加|插入).{0,36}(?:代码|文件|文档|Markdown|md|报告|src|docs)|(?:翻译|总结|摘要|概括|提取).{0,36}(?:成|为|到|至|入|保存|输出|写入).{0,48}(?:代码|文件|文档|Markdown|md|报告|目录|路径|src|docs|\/)|(?:代码实现|实现代码).{0,40}(?:创建于|放到|放入|输出到|写入到|保存到|目录|路径|\/)|(?:所有|全部|本次).{0,24}(?:新增|生成|输出|翻译|总结|摘要|概括|提取).{0,36}(?:文档|代码|文件).{0,48}(?:目录|路径|docs|src|\/)|(?:文档|代码|文件).{0,24}(?:放入|放到|输出到|写入到|保存到).{0,48}(?:目录|路径|docs|src|\/))/i;
const SCOPED_EXISTING_SOURCE_NO_CHANGE_RE = /(?:(?:不要|不用|无需|不需要|禁止|避免|不得|请勿|别).{0,16}(?:修改|改动|改|变更|覆盖|写入).{0,28}(?:正式源码|正式代码|正式源码目录|正式文件|原有代码|原代码|既有代码|既有文件|任何源码|所有源码|全部源码|源码|source\s+code|原项目|主项目|生产源码|生产代码)|(?:不修改|不改动|不覆盖).{0,28}(?:正式源码|正式代码|正式源码目录|正式文件|原有代码|原代码|既有代码|既有文件|任何源码|所有源码|全部源码|源码|source\s+code|原项目|主项目|生产源码|生产代码))/i;
...
const DELIVERABLE_PATH_ACTION_RE = /(?:新增|创建|新建|生成|编写|写入|输出|保存|落盘|放入|放到|写到|存到|翻译|总结|摘要|概括|提取|重命名|改名|移动|移到|挪到|挪动|复制|拷贝|追加|插入|create|generate|write|save|output|translate|summari[sz]e|rename|move|copy|append|insert)/gi;
