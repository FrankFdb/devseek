...
const UNAMBIGUOUS_EXTERNAL_EFFECT_RE = /(?:上线|部署|安装插件|安装扩展|提交(?:当前)?(?:修改|变更)|推送(?:当前)?(?:分支)|拉取(?:最新)?代码|安装[^，,。；;\n]{0,24}(?:依赖|npm\s*包|软件包|包|库|模块)|(?:新增|添加|引入)[^，,。；;\n]{0,16}(?:依赖|npm\s*包|软件包|包|库|模块)|\bdeploy\b|install\s+extension|\binstall\b[^,.;\n]{0,40}\b(?:packages?|dependenc(?:y|ies)|librar(?:y|ies)|modules?)\b|\binstall\s+@[A-Za-z0-9._/-]+\b|\binstall\s+[A-Za-z0-9][A-Za-z0-9._/-]{1,}\b(?=[,.;\n]|$|\s+(?:and|then|to)\b)|\b(?:add|introduce)\b[^,.;\n]{0,40}\b(?:new\s+)?dependenc(?:y|ies)\b|git\s+(?:commit|push|pull|fetch|merge|rebase)|commit\s+(?:changes?|current)|push\s+(?:(?:this|the|current)\s+)?branch|npm\s+(?:install|i|add|ci)|pnpm\s+(?:install|i|add)|yarn\s+(?:install|add)|pip\s+install)/i;
const AMBIGUOUS_RELEASE_ACTION_RE = /(?:发布|(?:^|[\s，,。；;:：])(?:release|publish)\b)/i;
...
const DOMAIN_INSTALL_ACTION_RE = /(?:安装[^，,。；;\n]{0,24}(?:处理器|回调|监听器|中间件)|\binstall\b[^,.;\n]{0,40}\b(?:handler|callback|listener|middleware)\b)/i;
...
export function classifyExternalEffectIntent(text: string): ExternalEffectIntent {
  ...
  for (const clause of splitOperationalClauses(text)) {
    ...
    if (DOMAIN_INSTALL_ACTION_RE.test(clause)) continue;
