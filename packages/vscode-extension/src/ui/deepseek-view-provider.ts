...
export class DeepSeekViewProvider implements vscode.WebviewViewProvider {
  ...
  private async handleApplyGeneratedFiles(wv: vscode.Webview, msg: WebviewMessage): Promise<void> {
    if (!msg.text) return;
    const runContext = createViewApplyRunContext(msg, 'vscode-extension.webview-apply-files');
    ...
    const reporter = async (status: ApplyWorkflowStatus) => {
      unresolvedFailure = updateViewWorkflowEvidence(runContext, status, unresolvedFailure);
      postWebviewEvent(wv, { kind: 'workflow', status });
    };
    const routeChat = (opts: ViewRouteChatOptions) => this.deps.routeChat({
      ...opts,
  ...
  private async handleApplyGeneratedPath(wv: vscode.Webview, msg: WebviewMessage): Promise<void> {
    ...
    const runContext = createViewApplyRunContext(msg, 'vscode-extension.webview-apply-path');
    const validationCommandRunner = this.deps.terminalPermissionCoordinator.createValidationCommandRunner({
      webview: wv,
      workspaceRoot: runContext.workspaceRoot,
      mode: 'edit',
...
function createViewApplyRunContext(msg: WebviewMessage, source: string) {
...
function updateViewWorkflowEvidence(
