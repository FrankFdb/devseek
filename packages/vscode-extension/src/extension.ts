...
function computePendingHunks(recordId: string, oldContent: string, newContent: string): PendingEditHunk[] {
  const oldLines = (oldContent || '').split('\n');
  const newLines = (newContent || '').split('\n');
...
function renderPendingContentFromHunks(record: PendingEditRecord): string {
...
function allHunksResolved(record: PendingEditRecord): boolean {
  return record.hunks.length > 0 && record.hunks.every((hunk) => hunk.resolution !== 'pending');
}
