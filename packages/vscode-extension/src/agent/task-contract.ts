...
const INSPECTION_RE = /(?:读取|提取|检查|审计|分析|列出|查看|总结|摘要|概括|翻译|只读|read|extract|inspect|audit|analy[sz]e|summari[sz]e|translate|take\s+a\s+look|look\s+at)/i;
...
const FILE_READ_ACTION_PATTERN = '(?:只读|读取|读出|查看|检查|审计|分析|解释|核对|参考|总结|摘要|概括|提取|翻译|\\b(?:read|inspect|view|check|audit|analy[sz]e|explain|extract|reference|translate)\\b|\\bsummari[sz]e\\b|\\btake\\s+a\\s+look\\b|\\blook\\s+at\\b)';
...
function classifyExplicitPathRole(
  ...
  const beforeRead = before.match(/(?:读取|查看|检查|审计|分析|解释|参考|根据|基于|依据|利用|使用|模板|输入|从)\s*[^，,。；;\n]{0,24}$/i)
    ...
    || before.match(/\b(?:read|inspect|view|check|audit|analy[sz]e|explain|extract|reference|using|use|from|based\s+on|according\s+to|template|input|take\s+a\s+look\s+at|look\s+at)\s+[^,.;\n]{0,28}$/i)
