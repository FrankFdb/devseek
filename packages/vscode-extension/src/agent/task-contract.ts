...
const INSPECTION_RE = /(?:读取|提取|检查|审计|分析|列出|查看|看下|看一下|总结|摘要|概括|翻译|只读|read|extract|inspect|audit|scan|analy[sz]e|summari[sz]e|translate|take\s+a\s+look|look\s+(?:at|through))/i;
const CHANGE_RE = /(?:修复|修正|修改|处理一下|解决|搞定|实现|新增|添加|重构|集成|落地|替换|重命名|改名|移动|移到|挪到|挪动|复制|拷贝|追加|插入|删除|移除|fix|repair|resolve|modify|implement|add|refactor|replace|rename|move|copy|append|insert|delete|remove)/i;
...
const ADVISORY_CHANGE_REQUEST_RE = /(?:(?:给出|提供|输出|列出|制定|梳理).{0,60}(?:建议|对策|方案|计划|思路|步骤|清单)|(?:建议|对策|思路).{0,40}(?:修复|修正|修改|重构|方案|计划)|(?:suggest|recommend|propose|list|outline|draft).{0,80}(?:ways?|options?|recommendations?|plan|steps?|approach|how\s+to|fix|repair|refactor)|(?:how\s+to|ways?\s+to).{0,48}(?:fix|repair|refactor|improve|change|update))/i;
...
const NO_SOURCE_CHANGE_RE = /(?:不要|禁止|无需|不允许|不得|别).{0,24}(?:修改|改|动|改动|触碰|修复|重命名|改名|移动|移到|挪到|挪动|复制|拷贝|追加|插入).{0,12}(?:源码|代码|文件)|(?:do not|don't|must not).{0,24}(?:modify|change|touch|fix|repair|rename|move|copy|append|insert).{0,12}(?:source|code|files?)/i;
...
function classifyExplicitPathRole(
  ...
  const beforeRead = before.match(/(?:读取|查看|看下|看一下|检查|审计|分析|解释|参考|根据|基于|依据|利用|使用|模板|输入|从)\s*[^，,。；;\n]{0,24}$/i)
    || before.match(/(?:总结|摘要|概括|翻译|提取)(?![^，,。；;\n]{0,16}(?:成|为|到|至|入)\s*$)\s*[^，,。；;\n]{0,24}$/i)
    || before.match(/\b(?:read|inspect|view|check|audit|scan|analy[sz]e|explain|extract|reference|using|use|from|based\s+on|according\s+to|template|input|take\s+a\s+look\s+at|look\s+(?:at|through))\s+[^,.;\n]{0,28}$/i)
...
function classifyPassivePathMutation(
  ...
  const before = prompt.slice(clauseStart, occurrence.index);
  const after = prompt.slice(occurrence.index + occurrence.path.length, clauseEnd);
  const passive = after.match(/^\s*["'\x60”’]?\s*(?:应当|应该|必须|可以|允许|may|must|shall|should|can)?\s*(?:被|be\s+)?(?:创建|新建|生成|写入|保存|修复|修正|解决|处理|更新|修改|编辑|删除|重命名|移动|复制|追加|插入|created\b|written\b|saved\b|fixed\b|repaired\b|resolved\b|updated\b|modified\b|edited\b|deleted\b|renamed\b|moved\b|copied\b|appended\b|inserted\b)/i);
...
function findTargetPronounProhibition(
  ...
  const patterns = [
    /(?:不要|不得|禁止|严禁|不可|不允许|别)(?:再)?\s*(?:创建|写入|修复|修正|解决|处理|更新|修改|改写|改动|编辑|覆盖|删除|删|移除|重命名|移动|复制|追加|插入)\s*(?:它|该文件|这个文件|上述文件|该文档|这个文档)/gi,
    /\b(?:do\s+not|don't|must\s+not|should\s+not|never)\s+(?:create|write|fix|repair|resolve|update|modify|revise|edit|overwrite|delete|remove|rename|move|copy|append|insert)\s+(?:it|that\s+file|this\s+file|that\s+document|this\s+document)\b/gi,
  ];
...
function isTargetExceptedFromFileProhibition(
  ...
  return extractPathOccurrences(prompt).some(occurrence => {
    ...
    return /(?:不要|不得|禁止|严禁|不可|不允许|勿|别)[^，。；;\n]{0,60}(?:创建|新建|生成|写|改|修复|修正|解决|处理|修改|变更|编辑|覆盖|删除|删|移除)[^，。；;\n]{0,60}(?:文件|文档|报告)/i.test(context)
      || /\b(?:do\s+not|don't|must\s+not|should\s+not|may\s+not|shall\s+not|never)\b[^,.;\n]{0,80}\b(?:create|write|save|generate|fix|repair|resolve|update|modify|change|edit|overwrite|delete|remove|touch|rename|move|copy|append|insert)\b[^,.;\n]{0,80}\bfiles?\b/i.test(context);
  });
}
...
function stripOutputStyleGuidanceForQuality(prompt: string): string {
  return String(prompt || '').replace(OUTPUT_STYLE_GUIDANCE_RE, ' ');
}
...
export function buildTaskContract(promptText: string): TaskContract {
  const prompt = extractCurrentUserRequest(promptText);
  const qualityPrompt = stripOutputStyleGuidanceForQuality(prompt);
  ...
  const protocol = PROTOCOL_RE.test(qualityPrompt);
  const interfaceContract = INTERFACE_RE.test(qualityPrompt);
  const communicationChain = COMMUNICATION_CHAIN_RE.test(qualityPrompt);
  const explicitModificationPlan = /(?:原有代码修改清单|代码修改清单|修改点清单|existing.?code modification plan)/i.test(qualityPrompt);
