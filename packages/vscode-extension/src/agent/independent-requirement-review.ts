  ...
  return [
    {
      ...
      content: [
        ...
        'For order, priority, FIFO/LIFO, minimum/maximum, best bid/ask, sorting, routing, or first/last requirements, build a multi-element execution trace before marking the check satisfied. Verify container traversal direction, comparator semantics, begin/end/prev usage, and tie ordering from the actual source lines.',
        'For every reject/error/invalid-input requirement, identify the exact rejected input scenario and caller-observable failure branch. Check duplicates, already-used ids, empty values, NaN/non-finite numbers, and <= 0 quantities when named. An early return of a value that legitimate success can also produce is not rejection.',
        'When the public API is fixed and its normal success result can be empty/no-op (for example a vector of trades), invalid or rejected input must use a distinguishable existing failure channel such as an exception; an empty success-shaped result is not enough.',
        ...
        'When mutable state is duplicated between an order container and an id index, verify every partial-fill branch updates the source read by remaining/cancel/best queries; stale secondary copies are correctness defects.',
