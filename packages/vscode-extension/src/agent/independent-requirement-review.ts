  ...
  return [
    {
      ...
      content: [
        ...
        'Before deciding an inventory check, privately enumerate every independent sentence, numbered item, and must/must-not clause inside its exact quote. A check is satisfied only when every binding clause is satisfied; never let compliance with one clause hide a violation of another.',
        ...
        'When the request requires decisions to be based on a measured or computed value and forbids fixed approximations, any fixed limiter that can change the decision violates the requirement even if the measured value is consulted later as a filter.',
        'Audit boundary arithmetic in the declared operand types before casts or assignments. Test minimum and maximum representable inputs, loop increments, range subtraction, and exact endpoints; a wider destination does not repair overflow that already occurred in a narrower expression.',
        'For coordinates in a half-open interval, evaluate both exact endpoints and every derived marker or label mapping. Mapping the maximum to end rather than end minus one is a concrete reachable defect.',
        ...
        'Set evidence_authority to reported-validation only when VALIDATION FACT explicitly supplies the observed execution result. Historical validation text inside ORIGINAL USER REQUIREMENTS is requirement context, not current validation authority. Otherwise use source-snapshot.',
        ...
        'VALIDATION FACT is bound to the current write cohort. Its concrete rerun result supersedes an older observation of the same command in ORIGINAL USER REQUIREMENTS; it does not prove unexercised behavior.',
