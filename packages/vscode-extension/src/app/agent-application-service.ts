...
export class AgentApplicationService {
  ...
  async handle(command: AgentCommand): Promise<AgentEvent[]> {
    switch (command.type) {
      ...
      case 'task.resume':
      case 'task.cancel':
        return [this.errorEvent(command.commandId, `Unsupported command: ${command.type}`)];
    }
  }
  ...
  private async handleChatCommand(command: Extract<AgentCommand, { type: 'chat.request' }>): Promise<AgentEvent[]> {
    ...
    try {
      ...
      const event = this.errorEvent(command.commandId, error instanceof Error ? error.message : String(error));
      events.push(event);
      this.emit(event);
      throw error;
    }
  ...
  private errorEvent(commandId: string | undefined, message: string): AgentEvent {
    return {
      type: 'error',
      eventId: this.newId(),
      commandId,
