...
import { isProceedWithPriorTaskRequest } from '../intent/continuation-intent';

const EXPLICIT_SESSION_REFERENCE_RE = /(?:继续|接着|把它|这个|上个|上次|刚才|刚刚|之前|前面|上一轮|上轮|同一(?:个)?\s*session|同一(?:个)?\s*(?:会话|对话)|在.+基础上|基于.+(?:原来|原有|已有|现有|之前|上次|刚才)|不要.+(?:重写|重新写|另写)|(?:单独)?重新写|重写了一个|另写了一个|为什么不.+(?:原来|原有|已有|现有|之前|上次|刚才))|(?:\b(?:continue|resume)\b)|(?:\b(?:previous|last)\s+(?:task|request|turn|version|file|one)\b)|(?:\b(?:same|current)\s+(?:task|request|file)\b)|(?:\b(?:based\s+on|from)\s+(?:the\s+)?(?:previous|existing|current)\b)/i;
const SESSION_SHORTHAND_ACTION_RE = /^\s*请?\s*(?:再|然后|另外|顺便|也|添加|加一个|改|改成|优化|修复|(?:please\s+)?(?:also|then|next|add|change|modify|update|fix)\b)/i;
...
export function isLikelySessionContinuation(prompt: string): boolean {
  ...
  if (isProceedWithPriorTaskRequest(text)) return true;
  if (EXPLICIT_SESSION_REFERENCE_RE.test(text)) return true;
  if (SESSION_SHORTHAND_ACTION_RE.test(text)) return !hasExplicitWorkspacePath(text);
