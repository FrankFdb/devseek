...
const paths = {
  registry: processPath('docs/process/devseek-r4-doc-process-identity-reconciliation.json'),
  schema: processPath('docs/process/devseek-r4-doc-process-identity-reconciliation.schema.json'),
  generatedView: processPath('docs/process/generated/devseek-r4-doc-process-identity-reconciliation.md'),
};
...
if (write && errors.length === 0) {
  try {
    registry = buildR4DocProcessIdentityReconciliation({ repoRoot });
    writeJson(paths.registry, registry);
    writeFile(paths.generatedView, renderR4DocProcessIdentityReconciliationMarkdown(registry));
  } catch (error) {
...
if (registry) {
  validationResult = validateR4DocProcessIdentityReconciliation(registry, { repoRoot });
  errors.push(...validationResult.errors);
}

if (registry) {
  const expectedView = renderR4DocProcessIdentityReconciliationMarkdown(registry);
