...
const paths = {
  registry: processPath('docs/process/devseek-r4-clean-runtime-limited-observation.json'),
  schema: processPath('docs/process/devseek-r4-clean-runtime-limited-observation.schema.json'),
  generatedView: processPath('docs/process/generated/devseek-r4-clean-runtime-limited-observation.md'),
};
...
if (write && errors.length === 0) {
  try {
    registry = buildR4CleanRuntimeLimitedObservation(buildOptions);
    writeJson(paths.registry, registry);
    writeFile(paths.generatedView, renderR4CleanRuntimeLimitedObservationMarkdown(registry));
  } catch (error) {
...
if (registry) {
  validationResult = validateR4CleanRuntimeLimitedObservation(registry, buildOptions);
  errors.push(...validationResult.errors);
}

if (registry) {
  const expectedView = renderR4CleanRuntimeLimitedObservationMarkdown(registry);
...
const result = {
  ...
  observation_sha256: registry?.observation_sha256 ?? null,
  errors,
};
