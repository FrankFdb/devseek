...
const paths = {
  registry: processPath('docs/process/devseek-r4-process-artifacts-aggregate.json'),
  schema: processPath('docs/process/devseek-r4-process-artifacts-aggregate.schema.json'),
  generatedView: processPath('docs/process/generated/devseek-r4-process-artifacts-aggregate.md'),
};
...
if (write && errors.length === 0) {
  try {
    registry = buildR4ProcessArtifactsAggregate({ repoRoot });
    writeJson(paths.registry, registry);
    writeFile(paths.generatedView, renderR4ProcessArtifactsAggregateMarkdown(registry));
  } catch (error) {
...
if (registry) {
  validationResult = validateR4ProcessArtifactsAggregate(registry, { repoRoot });
  errors.push(...validationResult.errors);
}

if (registry) {
  const expectedView = renderR4ProcessArtifactsAggregateMarkdown(registry);
...
const result = {
  ...
  aggregate_sha256: registry?.aggregate_sha256 ?? null,
  errors,
};
