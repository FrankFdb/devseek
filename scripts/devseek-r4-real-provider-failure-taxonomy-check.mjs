...
const paths = {
  registry: processPath('docs/process/devseek-r4-real-provider-failure-taxonomy.json'),
  schema: processPath('docs/process/devseek-r4-real-provider-failure-taxonomy.schema.json'),
  generatedView: processPath('docs/process/generated/devseek-r4-real-provider-failure-taxonomy.md'),
};
...
if (write && errors.length === 0) {
  try {
    registry = buildR4RealProviderFailureTaxonomy({ repoRoot });
    writeJson(paths.registry, registry);
    writeFile(paths.generatedView, renderR4RealProviderFailureTaxonomyMarkdown(registry));
  } catch (error) {
...
if (registry) {
  validationResult = validateR4RealProviderFailureTaxonomy(registry, { repoRoot });
  errors.push(...validationResult.errors);
}

if (registry) {
  const expectedView = renderR4RealProviderFailureTaxonomyMarkdown(registry);
...
const result = {
  ...
  taxonomy_sha256: registry?.taxonomy_sha256 ?? null,
  errors,
};
