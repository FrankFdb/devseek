...
test('current C0 ledger wiring has one ledger owner and complete consumer coverage without qualification effect', () => {
  ...
  assert.deepEqual(actual.counts, {
    ...
    governed_consumers: 8,
    governed_consumers_covered: 8,
    bypasses: 0,
...
test('runtime fails closed on consumer bypass, missing phase gate, or manual state file', () => {
  ...
  consumerBypass.counts.governed_consumers_covered = 7;
...
function loadSources() {
  return {
    ...
    sourceContents: {
      ...
      'scripts/devseek-c0-wiring-reconciliation-check.mjs': readText('scripts/devseek-c0-wiring-reconciliation-check.mjs'),
