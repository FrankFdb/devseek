...
const paths = {
  registry: processPath('docs/process/devseek-r4-live-user-way-holdout-matrix.json'),
  schema: processPath('docs/process/devseek-r4-live-user-way-holdout-matrix.schema.json'),
  generatedView: processPath('docs/process/generated/devseek-r4-live-user-way-holdout-matrix.md'),
};
...
if (write && errors.length === 0) {
  try {
    registry = buildR4LiveUserWayHoldoutMatrix({ repoRoot });
    writeJson(paths.registry, registry);
    writeFile(paths.generatedView, renderR4LiveUserWayHoldoutMatrixMarkdown(registry));
  } catch (error) {
...
if (registry) {
  validationResult = validateR4LiveUserWayHoldoutMatrix(registry, { repoRoot });
  errors.push(...validationResult.errors);
}

if (registry) {
  const expectedView = renderR4LiveUserWayHoldoutMatrixMarkdown(registry);
...
const result = {
  ...
  matrix_sha256: registry?.matrix_sha256 ?? null,
  errors,
};
