...
const paths = {
  registry: processPath('docs/process/devseek-r4-scenario-language-replay-corpus.json'),
  schema: processPath('docs/process/devseek-r4-scenario-language-replay-corpus.schema.json'),
  generatedView: processPath('docs/process/generated/devseek-r4-scenario-language-replay-corpus.md'),
};
...
if (write && errors.length === 0) {
  try {
    registry = buildR4ScenarioLanguageReplayCorpus({ repoRoot });
    writeJson(paths.registry, registry);
    writeFile(paths.generatedView, renderR4ScenarioLanguageReplayCorpusMarkdown(registry));
  } catch (error) {
...
if (registry) {
  validationResult = validateR4ScenarioLanguageReplayCorpus(registry, { repoRoot });
  errors.push(...validationResult.errors);
}

if (registry) {
  const expectedView = renderR4ScenarioLanguageReplayCorpusMarkdown(registry);
...
const result = {
  ...
  corpus_sha256: registry?.corpus_sha256 ?? null,
  errors,
};
