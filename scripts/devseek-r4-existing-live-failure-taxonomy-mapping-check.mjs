...
const paths = {
  registry: processPath('docs/process/devseek-r4-existing-live-failure-taxonomy-mapping.json'),
  schema: processPath('docs/process/devseek-r4-existing-live-failure-taxonomy-mapping.schema.json'),
  generatedView: processPath('docs/process/generated/devseek-r4-existing-live-failure-taxonomy-mapping.md'),
};
...
if (write && errors.length === 0) {
  try {
    registry = buildR4ExistingLiveFailureTaxonomyMapping({ repoRoot });
    writeJson(paths.registry, registry);
    writeFile(paths.generatedView, renderR4ExistingLiveFailureTaxonomyMappingMarkdown(registry));
  } catch (error) {
...
if (registry) {
  validationResult = validateR4ExistingLiveFailureTaxonomyMapping(registry, { repoRoot });
  errors.push(...validationResult.errors);
}

if (registry) {
  const expectedView = renderR4ExistingLiveFailureTaxonomyMappingMarkdown(registry);
...
const result = {
  ...
  mapping_sha256: registry?.mapping_sha256 ?? null,
  errors,
};
